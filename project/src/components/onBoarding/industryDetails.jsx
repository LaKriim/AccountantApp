...
import { TruncatedText, TruncatedTextShort, EllipsisText } from '../onBoarding/truncatedText';
...
const IndustryDetails = ({ onComplete }) => {
  ...
  const renderCurrentStep = () => {
        ...
        <>
          ...
          <Box sx={{ mt: 4 }}>
            ...
            <Autocomplete
              ...
              renderInput={(params) => <TextField {...params} placeholder="Search for other classifications" fullWidth />}
    ...
    return (
      <Box sx={{ mb: 3 }}>
        <Autocomplete
          options={ !industryCode?.length ? codeTree : industryCode[industryCode.length - 1]?.children || [] }
          getOptionLabel={(option) => option.label || ""}
          value={null}
          inputValue={inputValue}
          onInputChange={(_, newInputValue) => setInputValue(newInputValue)}
          ...
          renderInput={(params) => <TextField {...params} placeholder="Select an option" fullWidth />}
          isOptionEqualToValue={() => true}
          fullWidth
        />
      </Box>
